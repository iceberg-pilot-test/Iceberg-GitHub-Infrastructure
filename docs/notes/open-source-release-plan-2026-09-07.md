...
Bu proje bugün `iceberg-pilot-test` org'unu Terraform ile yöneten, çalışan bir sistem
...
- Dashboard markadan arındırma (Faz 2.3): `package.json`, UI başlıkları, localStorage anahtarları — mekanik `iceberg → tidyorg`.
...
- `imports.tf` — `Iceberg-GitHub-Infrastructure` import'u kuruluma özel; `.example` olarak ship et veya "sil/yeniden yaz" diye belgele.
- `config/repositories/Iceberg-GitHub-Infrastructure.yml:42-43` — hardcoded `iceberg-pilot-test/platform-admins` slug'ı; org-türetilmiş hale getir.
- `org-settings.tf` org-id `import` bloğu → kuruluma özel, `.example`/koşullu.

### 2.3 İsim değişimi: iceberg → tidyorg
...
- Repo adı `Iceberg-GitHub-Infrastructure` → `tidyorg` (dosya adı + referanslar).
...
`main.tf:11-16` HCP `cloud{}` bloğu sabit `iceberg-infra`'ya.
...
- Dokümanlar: iç-tarih/kişisel olanları **sil** (OZAN_CREW_NOTES, pilot-verification, tasks-*, implementation plan, plans-and-pricing, external-review) → hem iş azalır hem PII/iceberg hitleri gider. Kalıcı kullanıcı dokümanlarını İngilizce'ye çevir.
