...
>
> Genel proje adı **tidyorg** olacak (iceberg kalkıyor). Dashboard tarafındaki isim
> değişimini (paket adı, UI başlıkları, `iceberg.*` localStorage anahtarları →
...
- **Hemen başlayabilirsin (bağımlılık yok):** isim değişimi (iceberg → tidyorg), Docker
